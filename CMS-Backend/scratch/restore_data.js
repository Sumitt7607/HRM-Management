...
async function run() {
    ...
    const usersData = [
        { name: 'Angalakuduru Pavithra', email: 'pavithra@HRMlearning.com', role: 'employee', department: 'Engineering', position: 'Software Engineer', doj: '2025-01-01', status: 'Active', password: 'password123' },
        { name: 'Venugopal Byrapaneni', email: 'HRM.venu@gmail.com' }
    ];

    let pavithra = await User.findOne({ email: 'pavithra@HRMlearning.com' });
    if (!pavithra) {
        pavithra = await User.create(usersData[0]);
        console.log('Created Angalakuduru Pavithra');
    }

    await User.findOneAndUpdate({ email: 'HRM.venu@gmail.com' }, { name: 'Venugopal Byrapaneni' });
